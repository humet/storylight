...
/**
 * Workflow status STREAM endpoint for a series' create/next-chapter workflow
 * (`docs/05-backend/api.md` "Route Handlers"). Returns the CLIENT-SAFE progress
 * view only — parent-friendly labels, never raw prompts, internal stage keys, or
 * provider errors. Authorised via the workflow's family, not the URL.
 */
export async function GET(
  ...
  try {
    ...
    if (!view) {
      return NextResponse.json(
        { code: "INVALID_COMMAND", message: "That series could not be found." },
        { status: 404 },
      );
    }
