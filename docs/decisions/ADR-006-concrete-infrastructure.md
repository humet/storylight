...
| Image model routes | Character references: `google/gemini-3-pro-image` (M4, reference-conditioned). Routine chapter illustration + targeted repair: `bytedance/seedream-5.0-pro` (image-route v2, 2026-07-21 — identity/outfit/style parity with Gemini at ~⅓ cost, watermark removed via the adapter's bytedance `watermark:false` provider option). Premium escalation: `google/gemini-3-pro-image`. Vision review: `google/gemini-2.5-flash` multimodal. Image-input support is mandatory for ADR-003; the two providers use different gateway APIs (Gemini via the LANGUAGE API, Seedream via the dedicated IMAGE API) behind one `ChapterImageModel` port. | |
